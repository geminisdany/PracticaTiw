...
<body>
		...
		<div id="main-content">		
			...
			<div class="container-fluid">
				<c:set var="provLog" scope="session" value="${proveedorLogeado}"></c:set>
				<h1 id="registroC">Suscripciones a productos de <c:out value="${provLog.nombre}"></c:out></h1>
				<p>A continuaci&oacute;n se muestra el nombre y el email de los clientes suscritos a sus productos.</p>
				<c:set var="HaySuscrip" scope="session" value="${haySuscripciones}"></c:set>
				<c:choose>
					<c:when test="${HaySuscrip}">
						<c:set var="listaSuscripciones" scope="session" value="${listaSuscripcion}"></c:set>
						<div class="row">
							<div class="col-xs-4">
								<b>NOMBRE CLIENTE</b>
							</div>
							...
							<div class="col-xs-4">
								<b>PRODUCTO SUSCRITO</b>
							</div>
						</div>
						<c:forEach items="${listaSuscripciones}" var="suscripcion">
							<div class="row">
								<div class="col-xs-4">
									<c:out value="${suscripcion.cliente.nombre}"></c:out>
								</div>
								<div class="col-xs-4">
									<c:out value="${suscripcion.cliente.email}"></c:out>
								</div>
								<div class="col-xs-4">
									<c:out value="${suscripcion.producto.nombre}"></c:out>
								</div>
							</div>
						</c:forEach>
						<br>
					...
					<c:otherwise>
						<p>No hay suscripciones a productos.</p>
					</c:otherwise>
				</c:choose>	
			</div>
		</div>
